const NOTES=["C","C♯","D","D♯","E","F","F♯","G","G♯","A","A♯","B"];
    ...
    const roomParams=new URLSearchParams(location.search),rawRoomMode=roomParams.get("mode"),pageMode=rawRoomMode==="parent"?"host":rawRoomMode==="child"?"member":rawRoomMode,isRoomHost=pageMode==="host",isRoomMember=pageMode==="member";let roomCode=roomParams.get("room")||"";if(isRoomHost&&!/^\d{6}$/.test(roomCode)){roomCode=localStorage.getItem("tuner-room-code-v1")||"";if(!/^\d{6}$/.test(roomCode)){roomCode=String(Math.floor(100000+Math.random()*900000));localStorage.setItem("tuner-room-code-v1",roomCode)}}const roomTopic=/^\d{6}$/.test(roomCode)?`xianyin-${roomCode}-room-metronome-v1`:"";let metroBpm=window.metronome.getBpm(),metroRunning=false;const metroPage=document.querySelector("#metro-page"),metroValue=document.querySelector("#metro-bpm"),metroDot=document.querySelector("#metro-dot"),metroStart=document.querySelector("#metro-start");metroValue.textContent=metroBpm;
    ...
    document.querySelectorAll(".tempo-presets button").forEach(button=>button.onclick=()=>setMetro(Number(button.dataset.bpm)));
        ...
        let jamRooms;
        try{const saved=JSON.parse(localStorage.getItem(JAM_KEY));jamRooms=Array.isArray(saved)&&saved.every(room=>room&&typeof room.id==="string"&&Array.isArray(room.seats)&&room.seats.every(seat=>seat&&JAM_ROLE[seat.role])&&Number.isFinite(room.bpm))?saved:jamSeed()}catch(e){jamRooms=jamSeed()}
        ...
        const jamSave=()=>{try{localStorage.setItem(JAM_KEY,JSON.stringify(jamRooms))}catch(e){jamToast("浏览器无法保存进度，本次仍可继续演奏")}};
    ...
    document.querySelector("#create-room").onclick=()=>{let code=localStorage.getItem("tuner-room-code-v1")||"";if(!/^\d{6}$/.test(code)){code=String(Math.floor(100000+Math.random()*900000));localStorage.setItem("tuner-room-code-v1",code)}location.href=`?mode=host&room=${code}#metro`};document.querySelector("#join-room").onclick=()=>location.href="?mode=member#metro";
